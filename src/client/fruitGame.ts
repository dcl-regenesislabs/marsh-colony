...
// Poisonous fruit (issue #148): the Fruit01 model IS the poisonous fruit —
// catching one sickens the pet (server/state.ts's feedFromMinigame sets
// pet.sick), kicking off the sicknessErrand.ts -> pepitoChase.ts cure flow.
// A round only ever spawns 1 or 2 of them (a single catch already sickens the
// pet, so a per-fruit percentage put ~7 in every round). Their spawn times are
// picked when catching begins (see pickPoisonTimes) and spread across the
// round: each one is the next fruit to respawn once its time has passed.
...
function applyResults(): void {
  ...
  console.log(`[Client] feed result: caught=${caught} poisoned=${poisoned}`)
